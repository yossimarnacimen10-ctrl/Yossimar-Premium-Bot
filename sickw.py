...
class SickwClient:
    ...
    async def balance(self) -> dict:
        """Consulta el saldo disponible de la cuenta SICKW."""
        params = {
            "format": "json",
            "key": self.api_key,
            "action": "balance",
        ...
        try:
            ...
            raise SickwError(f"Error consultando saldo SICKW: {exc}") from exc

        try:
            data = response.json()
        except (ValueError, json.JSONDecodeError):
            raise SickwError("SICKW devolvió una respuesta de saldo no válida.")

        if not isinstance(data, dict):
            raise SickwError("Formato de saldo SICKW inesperado.")

        return data

    async def check(self, identifier: str, service_id: int | None = None) -> dict:
        ...
        params = {
            ...
            "key": self.api_key,
            "imei": identifier,
            "service": str(service_id if service_id is not None else self.service_id),
        }
        ...
        if data.get("status") == "error" or any(err in lower for err in obvious_errors):
            message = (
                data.get("message")
                or data.get("error")
                or data.get("result")
...
BLACKLIST_ALIASES = (
    ...
    "blacklist check", "blacklist result", "imei blacklist",
    "esn blacklist", "gsma blacklist", "gsma blacklist status",
    "lost stolen", "lost/stolen", "lost or stolen",
    "lost or stolen status", "device blacklist status"
)
...
def _find_value(normalized, aliases, default="No disponible"):
    def canonical(value: str) -> str:
        # SICKW alterna entre claves como ``blacklist_status``,
        # ``Blacklist Status`` y ``blacklistStatus`` según el endpoint.
    ...
    for alias in aliases:
        ...
        for key, value in normalized:
            ...
            if (
                key_clean == alias
                or _key_tail(key_clean) == alias
                or canonical(key_clean) == canonical(alias)
                or canonical(_key_tail(key_clean)) == canonical(alias)
            ):
                if value:
                    return value
    ...
    for alias in aliases:
        alias = alias.lower().strip()
        for key, value in normalized:
            if alias in key.lower() or canonical(alias) in canonical(key):
                if value:
                    return value
            if value and canonical(key).endswith(canonical(alias)):
                return value

    return default


def _find_blacklist_value(data: dict, normalized, default: str = "No disponible") -> str:
    """Extrae Blacklist incluso cuando SICKW lo devuelve anidado o serializado."""
    value = _find_value(normalized, BLACKLIST_ALIASES)
    if value != "No disponible":
        return value

    # Algunas respuestas incluyen el campo dentro de un JSON/texto combinado.
    try:
        blob = json.dumps(data, ensure_ascii=False)
    except Exception:
        blob = str(data)
    match = re.search(
        r"(?is)black\s*[_-]?\s*list(?:ed)?(?:\s+status|\s+result|\s+check)?"
        r"\s*[:=]\s*[\"']?([^,}\"'\r\n]+)",
        blob,
    )
...
def format_yossimar_report(data: dict, blacklist_data: dict | None = None) -> str:
    ...
    blacklist = _find_blacklist_value(data, normalized)
    if blacklist_data:
        blacklist = _find_blacklist_value(
            blacklist_data, _normalized_pairs(blacklist_data), blacklist
        )
    if blacklist == "No disponible":
        # Diagnóstico temporal: permite identificar la clave real que devuelve SICKW.
        # No registra la API key, solo los nombres de campos y valores de la respuesta.
        logger.warning("SICKW no devolvió Blacklist reconocible. Respuesta: %s", raw_preview(data))
