...
class Database:
    ...
    def ensure_user(self, telegram_id: int, username: str | None, first_name: str | None):
        ...
        with self._lock, self._connect() as conn:
            conn.execute(
                """INSERT INTO users
                   (telegram_id, username, first_name, balance, balance_units, created_at, updated_at)
                   VALUES (?, ?, ?, 0, 0, ?, ?)
    ...
    def set_user_active(self, telegram_id: int, active: bool):
        ...
        with self._lock, self._connect() as conn:
            conn.execute(
                """INSERT INTO users
                   (telegram_id, username, first_name, balance, balance_units, created_at, updated_at, is_active)
                   VALUES (?, NULL, NULL, 0, 0, ?, ?, ?)
                   ON CONFLICT(telegram_id) DO UPDATE SET
                     is_active=excluded.is_active,
                     updated_at=excluded.updated_at""",
                (telegram_id, now, now, 1 if active else 0)
            )
            conn.commit()
