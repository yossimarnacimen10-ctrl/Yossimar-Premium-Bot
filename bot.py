...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_prompts = {
        "admin_add_user": ("add", "➕ *Agregar usuario*\n\nEnvíame el ID de Telegram del usuario.\nEjemplo: `123456789`"),
        "admin_suspend_user": ("suspend", "⛔ *Suspender usuario*\n\nEnvíame el ID de Telegram del usuario."),
        "admin_activate_user": ("activate", "✅ *Activar usuario*\n\nEnvíame el ID de Telegram del usuario."),
        "admin_status_user": ("status", "👤 *Ver estado*\n\nEnvíame el ID de Telegram del usuario."),
        "admin_credit_user": ("credit", "💰 *Agregar créditos*\n\nEnvíame el ID y la cantidad separados por un espacio.\nEjemplo: `123456789 10`"),
        "admin_debit_user": ("debit", "💸 *Quitar créditos*\n\nEnvíame el ID y la cantidad separados por un espacio.\nEjemplo: `123456789 5`"),
    }
    if action in admin_prompts:
        if not is_admin(user_id):
            await query.answer("No autorizado.", show_alert=True)
            return
        state, prompt = admin_prompts[action]
        context.user_data["admin_action"] = state
        await query.edit_message_text(
            prompt,
            parse_mode=ParseMode.MARKDOWN,
        ...
        try:
            if admin_action == "add":
                ...
                if len(parts) != 1:
                    raise ValueError
                if is_owner(target):
                    await update.effective_message.reply_text("👑 El propietario principal no necesita suscripción.", reply_markup=admin_panel_menu())
                else:
            ...
            elif admin_action == "suspend":
                target = int(parts[0])
                if len(parts) != 1:
                    raise ValueError
                if is_owner(target):
                    await update.effective_message.reply_text("👑 El propietario principal no puede ser suspendido.", reply_markup=admin_panel_menu())
                else:
                    ...
                    try:
                        await context.bot.send_message(chat_id=target, text="⛔ Tu acceso a Yossimar Premium Bot ha sido suspendido. Contacta a soporte para renovar tu servicio.")
                    except Exception:
                        pass

            elif admin_action == "activate":
                target = int(parts[0])
                if len(parts) != 1:
                    raise ValueError
                ...
                await update.effective_message.reply_text(
                    f"✅ Usuario {target} activado por 30 días.\n📅 Vence: {format_expiry(expiry)}\n🎁 No se agregaron créditos gratis.",
                    reply_markup=admin_panel_menu(),
                )
                try:
                    await context.bot.send_message(chat_id=target, text=f"✅ Tu acceso a Yossimar Premium Bot ha sido renovado por 30 días.\n📅 Vence: {format_expiry(expiry)}\nTu saldo de créditos se mantiene igual.")
                except Exception:
                    pass
                ...
                expiry = db.get_subscription_expires_at(target)
                await update.effective_message.reply_text(
                    f"👤 Usuario {target}\nEstado: {status}\nVence: {format_expiry(expiry)}\nSaldo: {format_credits(db.get_balance(target))} créditos",
                    reply_markup=admin_panel_menu(),
                )
            ...
            examples = {
                "add": "Envía solo el ID. Ejemplo: `123456789`",
                "suspend": "Envía solo el ID. Ejemplo: `123456789`",
                "activate": "Envía solo el ID. Ejemplo: `123456789`",
                "status": "Envía solo el ID. Ejemplo: `123456789`",
                "credit": "Envía ID y cantidad. Ejemplo: `123456789 10`",
                "debit": "Envía ID y cantidad. Ejemplo: `123456789 5`",
            }
            await update.effective_message.reply_text(
                "❌ Formato inválido.\n\n" + examples.get(admin_action, "Inténtalo de nuevo."),
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=admin_cancel_menu(),
            )
            return
