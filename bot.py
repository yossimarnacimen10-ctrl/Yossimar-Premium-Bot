...
def _managed_users():
    """Devuelve usuarios administrables sin exigir cambios en db.py."""
    with db._lock, db._connect() as conn:
        params = []
        where = "(subscription_expires_at IS NOT NULL OR welcome_bonus_granted=1 OR is_active=1)"
        if settings.owner_id:
            where += " AND telegram_id<>?"
            params.append(settings.owner_id)
        return conn.execute(
            f"""SELECT telegram_id, username, first_name, is_active,
