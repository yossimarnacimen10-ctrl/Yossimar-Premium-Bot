...
async def perform_check(update: Update, context: ContextTypes.DEFAULT_TYPE, identifier: str):
    ...
    if not owner and not db.is_user_active(user_id):
        await update.effective_message.reply_text(
            "⛔ *Acceso suspendido*\n\n"
            "Tu acceso al bot está suspendido. Contacta a soporte para renovar tu servicio.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=main_menu(),
        )
        return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Cualquier mensaje compuesto exactamente por 15 dígitos se procesa
    # automáticamente como Apple Check, sin /check.
    if text.isdigit() and len(text) == 15:
        await perform_check(update, context, text)
        return

    # Los demás mensajes normales no consumen créditos ni llaman a SICKW.
...
async def admin_activate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        target = int(context.args[0])
        db.set_user_active(target, True)
        await update.effective_message.reply_text(
            f"✅ Usuario {target} activado nuevamente."
        )
        try:
            await context.bot.send_message(
                chat_id=target,
                text="✅ Tu acceso a Yossimar Premium Bot ha sido activado nuevamente."
            )
        except Exception:
            pass
    except Exception:
...
async def admin_user_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        target = int(context.args[0])
        status = "✅ ACTIVO" if db.is_user_active(target) else "⛔ SUSPENDIDO"
        await update.effective_message.reply_text(
