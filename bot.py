...
blacklist_service_id = settings.sickw_blacklist_service_id
...
async def perform_check(update: Update, context: ContextTypes.DEFAULT_TYPE, identifier: str):
    ...
    try:
        ...
        blacklist_data = await sickw.check(identifier, blacklist_service_id)
        db.add_check(user_id, identifier, settings.sickw_service_id, cost, "completed", raw_preview(data))
        report = format_yossimar_report(data, blacklist_data)
        if owner:
