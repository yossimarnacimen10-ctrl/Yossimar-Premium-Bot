...
from validators import is_valid_imei, normalize_identifier
...
async def perform_check(update: Update, context: ContextTypes.DEFAULT_TYPE, identifier: str):
    ...
    if not is_valid_imei(identifier):
        await update.effective_message.reply_text(
            "❌ IMEI inválido.\n\n"
            "Envía únicamente un IMEI de exactamente *15 dígitos*.\n"
            "Ejemplo: `123456789012345`",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=check_result_menu(),
        )
        return
    ...
    if owner:
        ...
        try:
            ...
            await wait_message.edit_text("❌ Saldo insuficiente.", reply_markup=check_result_menu())
            return

    try:
        data = await sickw.check(identifier, settings.sickw_service_id)
        ...
        await wait_message.edit_text(error_text, reply_markup=check_result_menu())
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "sickw_balance":
        if not is_admin(user_id):
            await query.answer("No autorizado.", show_alert=True)
            return
        try:
            data = await sickw.balance()
            balance = data.get("balance", data.get("credits", "No disponible"))
            currency = data.get("currency", "")
            extra = f" {currency}" if currency else " créditos"
            text = (
                "🔋 *Créditos disponibles SICKW*\n\n"
                f"💳 Saldo actual: `{balance}`{extra}\n\n"
                f"🕒 Consulta: {datetime.now(timezone.utc).strftime('%d/%m/%Y %H:%M UTC')}"
            )
        except Exception as exc:
            logger.exception("Error consultando saldo SICKW")
            text = f"❌ No se pudo consultar el saldo SICKW.\n\n{exc}"
    ...
    if action == "history":
        ...
        await query.edit_message_text(text, parse_mode=ParseMode.MARKDOWN, reply_markup=menu_for(user_id))
        return
    ...
    if action.startswith("support_no_"):
        ...
        try:
            await context.bot.send_message(
                ...
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=menu_for(target_id),
            )
        except Exception:
            pass
        ...
        try:
            if admin_action == "add":
                ...
                if is_owner(target):
                    ...
                    bonus_text = "🎁 Créditos iniciales: 0.00 (recarga para comenzar)"
                    await update.effective_message.reply_text(
                    ...
                    try:
                        await context.bot.send_message(
                            ...
                            text=(
                                ...
                                + "🎁 Saldo inicial: 0.00 créditos. Recarga para comenzar.\n"
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Maneja errores conocidos sin tumbar el bot.

    - Message is not modified: ocurre al pulsar dos veces una pantalla idéntica.
    - Conflict/getUpdates: puede aparecer unos segundos durante un redeploy,
      mientras Telegram termina de soltar la instancia anterior.
    """
    error = context.error
    ...
    if isinstance(error, Conflict):
        logger.warning(
            "Telegram reportó un conflicto temporal de getUpdates. "
            "El bot seguirá intentando conectarse mientras termina la instancia anterior."
        )
        return
